...
import { useReactionBack } from "../hooks/use-reaction-back";
...
// Matches packages/schemas/emcee/v1.json's reaction_back.cooldown_s bounds —
...
const REACTION_BACK_FORM_ID = "avatar-reaction-back-form";

/**
 * Avatar → Reaction back card — the whole card, chrome included. Fully
 * self-contained: fetches its own current enabled/cooldown via
 * useReactionBack and owns its own save action, rather than being handed
 * `config.reaction_back` down from the page's server fetch. Rendered
 * directly in instance-config.tsx rather than driven through
 * `sections`/`SectionCard` — same move as AnchorSpotCard/IdleEmoteLoopCard
 * (docs/decisions.md, 2026-07-23): its own title/description are
 * hand-written here instead of schema-derived. A single function, like Idle
 * emote loop — `enabled` moves with it here rather than staying on the
 * generic form.
 */
export function ReactionBackCard({ instanceId }: { instanceId: string }) {
  const t = useTranslations("instanceDetail.config");
  const tInstance = useTranslations("instanceDetail");
  const { data, state, formAction } = useReactionBack(instanceId);
  ...
  // which would snap cooldown_s back to its pre-save value the instant a
  ...
  return (
    <div className="rounded-2xl border border-paper/10 bg-panel p-6">
      <h3 className="font-display text-base text-paper">{t("reactionBack.title")}</h3>
      <p className="mt-1 text-sm text-dust">{t("reactionBack.description")}</p>
      ...
      {!data ? (
        ...
        <div className="mt-5 grid gap-5">
          <div className="flex items-start gap-3">
            <Checkbox
              id="reaction-back-enabled"
              form={REACTION_BACK_FORM_ID}
              name="enabled"
              checked={enabled}
            ...
            />
            <Label htmlFor="reaction-back-enabled" className="font-normal leading-5 text-paper">
              {t("reactionBack.enabledLabel")}
            </Label>
          </div>

          <div className={`grid gap-5 border-t border-paper/10 pt-5 ${enabled ? "" : "hidden"}`}>
            <div className="grid gap-2">
              <Label htmlFor="reaction-back-cooldown" className="text-dust">
                {t("reactionBack.cooldownLabel")}
              </Label>
              <Input
                id="reaction-back-cooldown"
                form={REACTION_BACK_FORM_ID}
                name="cooldown_s"
                type="number"
                min={COOLDOWN_MIN}
              ...
              />
              <p className="text-xs text-dust">{t("reactionBack.cooldownDescription")}</p>
            </div>
      ...
      {portalTarget &&
        createPortal(<form id={REACTION_BACK_FORM_ID} ref={formRef} action={formAction} />, portalTarget)}
    </div>
  );
}
