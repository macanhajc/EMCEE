...
// Matches packages/schemas/emcee/v1.json's anti_spam bounds — this card no
...
const MESSAGE_RATE_COUNT_MIN = 3;
const MESSAGE_RATE_COUNT_MAX = 20;
const MESSAGE_RATE_WINDOW_MIN = 5;
const MESSAGE_RATE_WINDOW_MAX = 60;
const DUPLICATE_COUNT_MIN = 2;
const DUPLICATE_COUNT_MAX = 10;
...
const ANTI_SPAM_FORM_ID = "moderation-anti-spam-form";

/**
 * Moderation → Anti-spam card — the whole card, chrome included. Fully
 * self-contained: fetches its own current enabled/rate settings via
 * useAntiSpam and owns its own save action, rather than being handed
 * `config.anti_spam` down from the page's own server fetch. Rendered
 ...
 */
export function AntiSpamCard({ instanceId }: { instanceId: string }) {
  const t = useTranslations("instanceDetail.config");
  const tInstance = useTranslations("instanceDetail");
  const { data, state, formAction } = useAntiSpam(instanceId);
  ...
  // which would snap these back to their pre-save values the instant a save
  // completes (this hook's own refetch hasn't necessarily landed yet at that
  // exact point).
  const [enabled, setEnabled] = useState(false);
  const [messageRateCount, setMessageRateCount] = useState<number | "">("");
  const [messageRateWindowS, setMessageRateWindowS] = useState<number | "">("");
  ...
  if (data !== null && data !== seededFor) {
    ...
    setMessageRateCount(data.message_rate_count);
    setMessageRateWindowS(data.message_rate_window_s);
  ...
  return (
    <div className="rounded-2xl border border-paper/10 bg-panel p-6">
      <h3 className="font-display text-base text-paper">{t("antiSpam.title")}</h3>
      <p className="mt-1 text-sm text-dust">{t("antiSpam.description")}</p>
      ...
      {!data ? (
        ...
        <div className="mt-5 grid gap-5">
          <div className="flex items-start gap-3">
            ...
            />
            <Label htmlFor="anti-spam-enabled" className="font-normal leading-5 text-paper">
              {t("antiSpam.enabledLabel")}
            </Label>
          </div>

          <div className={`grid gap-5 border-t border-paper/10 pt-5 ${enabled ? "" : "hidden"}`}>
            <div className="grid gap-2">
              <Label htmlFor="anti-spam-message-rate-count" className="text-dust">
                {t("antiSpam.messageRateCountLabel")}
              </Label>
              <Input
                id="anti-spam-message-rate-count"
                form={ANTI_SPAM_FORM_ID}
                name="message_rate_count"
                type="number"
                min={MESSAGE_RATE_COUNT_MIN}
                max={MESSAGE_RATE_COUNT_MAX}
                value={messageRateCount}
                onChange={(e) => setMessageRateCount(e.target.value === "" ? "" : Number(e.target.value))}
                className={`h-10 max-w-40 ${fieldControlClass}`}
              />
              <p className="text-xs text-dust">{t("antiSpam.messageRateCountDescription")}</p>
            </div>

            <div className="grid gap-2">
              <Label htmlFor="anti-spam-message-rate-window" className="text-dust">
                {t("antiSpam.messageRateWindowLabel")}
              </Label>
              <Input
                id="anti-spam-message-rate-window"
                form={ANTI_SPAM_FORM_ID}
                name="message_rate_window_s"
                type="number"
                min={MESSAGE_RATE_WINDOW_MIN}
                max={MESSAGE_RATE_WINDOW_MAX}
                value={messageRateWindowS}
                onChange={(e) => setMessageRateWindowS(e.target.value === "" ? "" : Number(e.target.value))}
                className={`h-10 max-w-40 ${fieldControlClass}`}
              />
            </div>

            <div className="grid gap-2">
              <Label htmlFor="anti-spam-duplicate-count" className="text-dust">
                {t("antiSpam.duplicateCountLabel")}
              </Label>
              <Input
                id="anti-spam-duplicate-count"
                form={ANTI_SPAM_FORM_ID}
                ...
                onChange={(e) => setDuplicateCount(e.target.value === "" ? "" : Number(e.target.value))}
                className={`h-10 max-w-40 ${fieldControlClass}`}
              />
              <p className="text-xs text-dust">{t("antiSpam.duplicateCountDescription")}</p>
            </div>
      ...
      {portalTarget && createPortal(<form id={ANTI_SPAM_FORM_ID} ref={formRef} action={formAction} />, portalTarget)}
    </div>
  );
}
