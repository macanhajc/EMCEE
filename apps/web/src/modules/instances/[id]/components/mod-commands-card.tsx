...
import { useModCommands } from "../hooks/use-mod-commands";
...
// Matches packages/schemas/emcee/v1.json's commands.prefix bounds (a single
// character) — this card no longer goes through sectionsFromSchema, so this
// is hand-kept in sync rather than derived, same tradeoff the other
// extracted cards already made for their own schema-derived bounds/enums.
const PREFIX_MAX_LENGTH = 1;
...
const MOD_COMMANDS_FORM_ID = "moderation-mod-commands-form";

/**
 * Moderation → Mod commands card (schema section key `commands`) — the
 * whole card, chrome included. Fully self-contained: fetches its own
 * current enabled/prefix via useModCommands and owns its own save action,
 * rather than being handed `config.commands` down from the page's own
 * server fetch. Rendered directly in instance-config.tsx rather than driven
 * through `sections`/`SectionCard` — same move as the Avatar module's cards
 * (docs/decisions.md, 2026-07-24).
 */
export function ModCommandsCard({ instanceId }: { instanceId: string }) {
  const t = useTranslations("instanceDetail.config");
  const tInstance = useTranslations("instanceDetail");
  const { data, state, formAction } = useModCommands(instanceId);
  ...
  return (
    <div className="rounded-2xl border border-paper/10 bg-panel p-6">
      <h3 className="font-display text-base text-paper">{t("modCommands.title")}</h3>
      <p className="mt-1 text-sm text-dust">{t("modCommands.description")}</p>
      ...
      {!data ? (
        ...
        <div className="mt-5 grid gap-5">
          <div className="flex items-start gap-3">
            <Checkbox
              id="mod-commands-enabled"
              form={MOD_COMMANDS_FORM_ID}
              name="enabled"
              checked={enabled}
            ...
            />
            <Label htmlFor="mod-commands-enabled" className="font-normal leading-5 text-paper">
              {t("modCommands.enabledLabel")}
            </Label>
          </div>

          <div className={`grid gap-5 border-t border-paper/10 pt-5 ${enabled ? "" : "hidden"}`}>
            <div className="grid gap-2">
              <Label htmlFor="mod-commands-prefix" className="text-dust">
                {t("modCommands.prefixLabel")}
              </Label>
              <Input
                id="mod-commands-prefix"
                form={MOD_COMMANDS_FORM_ID}
                name="prefix"
                type="text"
                required
                ...
                className={`h-10 max-w-24 ${fieldControlClass}`}
              />
            </div>
      ...
      {portalTarget &&
        createPortal(<form id={MOD_COMMANDS_FORM_ID} ref={formRef} action={formAction} />, portalTarget)}
    </div>
  );
}
