...
const FILTER_FORM_ID = "moderation-filter-form";

/**
 * Moderation → Word filter card — the whole card, chrome included. Fully
 * self-contained: fetches its own current enabled/custom_terms via useFilter
 * and owns its own save action, rather than being handed `config.filter`
 ...
 * same move as the Avatar module's cards (docs/decisions.md, 2026-07-24).
 */
export function FilterCard({ instanceId }: { instanceId: string }) {
  const t = useTranslations("instanceDetail.config");
  const tInstance = useTranslations("instanceDetail");
  const { data, state, formAction } = useFilter(instanceId);
  ...
  // edit. See anchor-spot-card.tsx for the same pattern.
  ...
  return (
    <div className="rounded-2xl border border-paper/10 bg-panel p-6">
      <h3 className="font-display text-base text-paper">{t("filter.title")}</h3>
      <p className="mt-1 text-sm text-dust">{t("filter.description")}</p>
      ...
      {!data ? (
        ...
        <div className="mt-5 grid gap-5">
          <div className="flex items-start gap-3">
            ...
            />
            <Label htmlFor="filter-enabled" className="font-normal leading-5 text-paper">
              {t("filter.enabledLabel")}
            </Label>
          </div>

          <div className={`grid gap-5 border-t border-paper/10 pt-5 ${enabled ? "" : "hidden"}`}>
            <div className="grid gap-2">
              <Label htmlFor="filter-custom-terms" className="text-dust">
                {t("filter.customTermsLabel")}
              </Label>
              <TagListInput
                id="filter-custom-terms"
                name="custom_terms"
                form={FILTER_FORM_ID}
                defaultValue={data.custom_terms}
                placeholder={t("tagListPlaceholder")}
              />
              <p className="text-xs text-dust">{t("filter.customTermsDescription")}</p>
            </div>
      ...
      {portalTarget && createPortal(<form id={FILTER_FORM_ID} ref={formRef} action={formAction} />, portalTarget)}
    </div>
  );
}
