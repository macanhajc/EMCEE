...
import {
  getExemptionsConfig,
  updateExemptionsConfig,
  type ConfigActionState,
} from "@/app/[locale]/instances/[id]/actions";
import { useInstanceStore } from "../store";

/**
 * Owns the Moderation → Exemptions card end to end — query reads from the
 * shared instance store (populated once by store.ts's `loadAll`, not
 * fetched here) and mutate (`updateExemptionsConfig`, via `useActionState`)
 ...
 * `exemptions-card.tsx` is the only consumer.
 */
export function useExemptions(instanceId: string) {
  const data = useInstanceStore((s) => s.exemptions);
  const setSection = useInstanceStore((s) => s.setSection);

  const [state, formAction] = useActionState(
    (prevState: ConfigActionState | null, formData: FormData) =>
      updateExemptionsConfig(instanceId, prevState, formData),
    null,
  );
  ...
  useEffect(() => {
    if (state?.ok) getExemptionsConfig(instanceId).then((result) => setSection("exemptions", result));
