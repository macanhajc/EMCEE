...
import {
  getEmoteAllConfig,
  updateEmoteAllConfig,
  type ConfigActionState,
} from "@/app/[locale]/instances/[id]/actions";
import { useInstanceStore } from "../store";

/**
 * Owns the Emotes → Emote all card end to end — query reads from the shared
 * instance store (populated once by store.ts's `loadAll`, not fetched here)
 * and mutate (`updateEmoteAllConfig`, via `useActionState`) writes the
 ...
 * "instance store"). Mirrors every other card's hook; `emote-all-card.tsx`
 * is the only consumer.
 */
export function useEmoteAll(instanceId: string) {
  const data = useInstanceStore((s) => s.emoteAll);
  const setSection = useInstanceStore((s) => s.setSection);

  const [state, formAction] = useActionState(
    (prevState: ConfigActionState | null, formData: FormData) =>
      updateEmoteAllConfig(instanceId, prevState, formData),
    null,
  );
  ...
  useEffect(() => {
    if (state?.ok) getEmoteAllConfig(instanceId).then((result) => setSection("emoteAll", result));
