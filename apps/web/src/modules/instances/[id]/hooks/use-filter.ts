...
/**
 * Owns the Moderation → Word filter card end to end — query reads from the
 * shared instance store (populated once by store.ts's `loadAll`, not
 * fetched here) and mutate (`updateFilterConfig`, via `useActionState`)
 ...
 * `filter-card.tsx` is the only consumer.
 */
export function useFilter(instanceId: string) {
  const data = useInstanceStore((s) => s.filter);
  const setSection = useInstanceStore((s) => s.setSection);

  const [state, formAction] = useActionState(
    (prevState: ConfigActionState | null, formData: FormData) =>
      updateFilterConfig(instanceId, prevState, formData),
    null,
  );
  ...
  useEffect(() => {
    if (state?.ok) getFilterConfig(instanceId).then((result) => setSection("filter", result));
