...
import { InstanceStatusBadge, SubscriptionBadge } from "@/components/Elements/instance-status";
  ...
  return (
    <div className="flex flex-col gap-4 rounded-2xl border border-paper/10 bg-panel p-6 sm:flex-row sm:items-center sm:justify-between">
      <div>
        ...
        <p className="mt-1 font-ui-mono text-xs text-dust">{t("room", { roomId: instance.roomId })}</p>
        <div className="mt-2">
          {subscription ? (
      ...
      >
        <Link href={`/instances/${instance.id}`}>{t("manage")}</Link>
      </Button>
    </div>
  );
}
